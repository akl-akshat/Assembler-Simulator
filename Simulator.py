f = open("C:\\Users\\aradh\\OneDrive\\Desktop\\input.txt", "r")
lines = f.readlines()
if not lines:
...
if not programCounter:
    ...
    exit()
OUTPUTS=[]
r=open("C:\\Users\\aradh\\OneDrive\\Desktop\\output.txt",'w')
def bin32(num):
    binary = bin(num)[2:]
...
def itype(instruction, rd, rs1, immediate,func3,oprogramCounterode,programCounter_Execution):
    ...
    if func3=='000' and oprogramCounterode=='0010011': 
        ...
        registerValue[rd] = programCounter_Execution + 4
        programCounter_Execution = registerValue[rs1] + imm
    else:
        raise ValueError("Unsupported I-type instruction")
        
def btype(instruction,rs1,rs2,immediate,func3,programCounter_Execution):
    imm=bintodec(immediate)
    if  func3 =="000":
         offset = (imm << 1) | 0b0
         target_address = programCounter_Execution + offset
         if rs1 == rs2:  
            return target_address, True  
         else:
            return programCounter_Execution + 4, False
    if  func3 =="001":
         offset = (imm << 1) | 0b0  
         target_address = programCounter_Execution + offset  
...
def stype(instruction,rs2,rs1,immediate,func3,oprogramCounterode,programCounter_Execution):
    imm = bintodec(immediate)
    if func3 == "010":
...
def utype(instruction,  rd, imm, oprogramCounterode, programCounter_Execution):
    imm_decimal = bintodec(imm)
    if oprogramCounterode=="0110111":
        registerValue[rd] = imm_decimal << 12
    elif oprogramCounterode=="0010111":
        registerValue[rd] = programCounter_Execution + (imm_decimal << 12)
    else:
        raise ValueError("Unsupported U-type instruction")


def jtype(instruction, rd, imm, oprogramCounterode, programCounter_Execution):
    imm_decimal = bintodec(imm)
    registerValue[rd] = programCounter_Execution + 4
    offset = ((int(imm) & 0b111111111111) << 1) | 0b0
    target_address = programCounter_Execution + offset
    target_address &= 0xFFFFFFFE
    programCounter_Execution = target_address
    
    
def programCounter_AND_ALL_REGS_OUTPUT(programCounter_Execution):
    output = '0b'+ str(bin32(programCounter_Execution)) + " " 
    register_bin_values = {}
...
def execute_instruction(instruction,programCounter_Execution):
    ...
    if type == "R":
        ...
        itype(instruction, instruction[20:25], instruction[12:17], instruction[0:12],instruction[17:20],instruction[25:32],programCounter_Execution)
    elif type == "B":
        imm=instruction[0]+instruction[24]+instruction[1:7]+instruction[20:26]
        btype(instruction,instruction[12:17],instruction[20:25],imm,instruction[17:20],programCounter_Execution)
    elif type == "S":
        imm = instruction[0:7] + instruction[20:25]
        stype(instruction, instruction[7:12], instruction[12:17], imm, instruction[17:20],instruction[25:31],programCounter_Execution)
    elif type == "U":
        utype(instruction,instruction[20:25],instruction[0:20],instruction[25:32],programCounter_Execution)
    elif type == "J":
        imm = instruction[0] + instruction[12:20] + instruction[11] + instruction[1:11]
        jtype(instruction,instruction[20:25],imm,instruction[25:32],programCounter_Execution)
programCounter_end = max(programCounter.keys())
programCounter_Execution = 0 

while programCounter_Execution < programCounter_end:
    instruction = programCounter[programCounter_Execution]  
    execute_instruction(instruction, programCounter_Execution)
    programCounter_AND_ALL_REGS_OUTPUT(programCounter_Execution)
    programCounter_Execution += 4
